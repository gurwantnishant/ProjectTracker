...
const Tasks = (() => {
  ...
  function bindSubtaskControls(root) {
    ...
    function updateProgressUI() {
      ...
      if (total) {
        ...
        // Mirror readForm's auto-status logic live: all subtasks done bumps
        // Status to Completed; reopening one reverts an auto-completed
  ...
  function readForm(root) {
    ...
    const progress = subtasks.length
      ...
      : manualProgress;
    // Mirrors the milestone auto-status logic (see storage.js setTasks): when
    // a task has subtasks and every one of them is checked off, the task's
    // own status is bumped to 'Completed' automatically. If a subtask gets
    // reopened afterward, a status of 'Completed' reverts to 'In Progress'
    // rather than staying stuck at 100%. Tasks without subtasks keep a fully
    // manual status.
    const manualStatus = root.querySelector('#f_status').value;
    let status = manualStatus;
    if (subtasks.length) {
      const allDone = subtasks.every(s => s.completed);
      if (allDone) status = 'Completed';
      else if (manualStatus === 'Completed') status = 'In Progress';
    }
    return {
